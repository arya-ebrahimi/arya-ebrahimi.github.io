...
The policy $\pi(a \| s) : \mathcal{M} \times \mathcal{A} \mathbb{R}_{+}$ is a distribution over actions conditioned on states. The interaction of the policy with the environment creates a distribution over trajectories, which is defined as:
